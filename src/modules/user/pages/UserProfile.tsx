...
import { getUserDetails, updateUserProfile } from '../../../store/actions/user/userActions';
import { showToast, showErrorAlert } from '../../../common/utils/alertUtils';

import styles from '../../../schemas/css/UserProfile.module.css';
...
const UserProfile = () => {
    ...
    // --- REDUX STATE ---
    const userAuth = useSelector((state: RootState) => state.userAuth as any);
    const { userInfo } = userAuth || {};

    const userDetails = useSelector((state: RootState) => state.userDetails as any);
    const { loading, error, user } = userDetails || {};
    const currentUser: any = user || {};

    const userUpdateProfile = useSelector((state: RootState) => state.userUpdateProfile as any);
    ...
    useEffect(() => {
        if (!userInfo) {
            ...
            if (success) {
                ...
                // Populate the states when user data is loaded
                setName(currentUser.name);
                setCity(currentUser.address?.city || '');
                setStateName(currentUser.address?.state || '');
        ...
        if (updateError) {
            ...
            dispatch({ type: 'USER_UPDATE_PROFILE_RESET' }); // Clear error so they can try again
        }

    }, [dispatch, navigate, userInfo, currentUser, success, updateError, error, loading]);
                    ...
                    <div className={styles['profile-body']}>
                        ...
                        <div>
                            ...
                            <div className={styles['info-group']}>
                                ...
                                <div className={styles['info-value']}>{currentUser.email}</div>
                            </div>
                            <div className={styles['info-group']}>
                                <span className={styles['info-label']}>Mobile Number</span>
                                <div className={styles['info-value']}>+91 {currentUser.mobile}</div>
                            </div>
                        </div>

                        {/* Address Info */}
                        <div>
                            <h3 className={styles['section-title']}>Default Address</h3>
                            <div className={styles['info-group']}>
                                <span className={styles['info-label']}>City</span>
                                <div className={styles['info-value']}>{currentUser.address?.city || 'Not provided'}</div>
                            </div>
                            <div className={styles['info-group']}>
                                <span className={styles['info-label']}>State</span>
                                <div className={styles['info-value']}>{currentUser.address?.state || 'Not provided'}</div>
                            </div>
                            <div className={styles['info-group']}>
                                <span className={styles['info-label']}>Pincode</span>
                                <div className={styles['info-value']}>{currentUser.address?.pincode || 'Not provided'}</div>
                            </div>
                        </div>
                    ...
                    <div className={styles['modal-backdrop']} onClick={() => setIsEditProfileOpen(false)}>
                        <div className={styles['modal-content']} onClick={(e) => e.stopPropagation()}>
                            <div className={styles['modal-header']}>
                                <h2 className={styles['modal-title']}>Edit Profile</h2>
